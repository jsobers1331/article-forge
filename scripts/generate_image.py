...
def generate_image(prompt, out_path, provider="openai", model=None, size=None, timeout=120):
    ...
    return {"path": out_path, "input_tokens": in_tok, "output_tokens": out_tok, "cost_usd": cost}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate one supporting image and convert to WebP")
    parser.add_argument("--prompt-file", required=True, help="Path to a text file with the image prompt")
    ...
    result = generate_image(prompt_text, tmp_png, provider=args.provider, model=args.model, size=args.size)
    size_bytes = convert_to_webp(tmp_png, args.out)
    os.remove(tmp_png)

    print(f"Saved {args.out} ({size_bytes / 1024:.1f} KB)")
    print(f"Tokens: in={result['input_tokens']} out={result['output_tokens']} | cost ~${result['cost_usd']:.4f}")
    print("Now run the QC checklist in IMAGES.md §6 before shipping — do not skip it.")
