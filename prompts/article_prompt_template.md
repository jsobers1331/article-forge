...
8. Target length: {target_length} words. Stop when the question is fully answered — do not pad.
...
16. Internal linking: include 3-5 Markdown links to the existing site pages listed above, placed naturally in-line (never a bare link list) — at least one in the first third of the article, not only in the closing CTA.
17. External linking: the first time you name a competitor, link to its real URL from the list above. Don't add external links beyond the named competitors — no invented "sources" or citations you can't verify.
